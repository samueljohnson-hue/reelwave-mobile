...
<div class="clipline"><span>Selected</span><strong id="clipRead">0:00 – 0:50</strong></div><div class="tiny">Every lyric video must be between 30 and 50 seconds. Move the start point to choose the verse, chorus or part you want.</div></div></section>''', 1)
s = s.replace('.range{width:100%}video{', '.range{width:100%;accent-color:#8da2ff}.clipbox{background:#0e1221;border:1px solid #2d3554;border-radius:16px;padding:14px;margin-top:14px}.clipline{display:flex;justify-content:space-between;gap:12px;font-size:13px;margin:8px 0}video{', 1)
s = s.replace("shareBtn=$('shareBtn');", "shareBtn=$('shareBtn'),clipBox=$('clipBox'),clipStartEl=$('clipStart'),clipLengthEl=$('clipLength'),startRead=$('startRead'),lengthRead=$('lengthRead'),clipRead=$('clipRead');", 1)
s = s.replace('beatPulse=0,lastBeat=0;', 'beatPulse=0,lastBeat=0,songDuration=0,clipStart=0,clipLength=50;', 1)
...
function updateClip(resetAI=true){
  ...
  const remaining=songDuration-clipStart,maxLen=Math.min(50,remaining);
...
function encodeAnalysisWav(buffer,startSec,durationSec){
  const targetRate=16000,srcRate=buffer.sampleRate,srcStart=Math.floor(startSec*srcRate),outFrames=Math.max(1,Math.floor(durationSec*targetRate));
  const out=new ArrayBuffer(44+outFrames*2),v=new DataView(out),str=(o,x)=>{for(let i=0;i<x.length;i++)v.setUint8(o+i,x.charCodeAt(i))};
  ...
  for(let i=0;i<outFrames;i++){const srcPos=srcStart+(i*srcRate/targetRate),i0=Math.floor(srcPos),i1=Math.min(i0+1,buffer.length-1),f=srcPos-i0;let sample=0;for(const ch of channels){const a=ch[Math.min(i0,ch.length-1)]||0,b=ch[Math.min(i1,ch.length-1)]||0;sample+=a+(b-a)*f}sample/=Math.max(1,channels.length);sample=Math.max(-1,Math.min(1,sample));v.setInt16(44+i*2,sample<0?sample*32768:sample*32767,true)}
...
'''
if needle not in s: raise SystemExit('Could not insert clip helper functions')
...
if old_ai_clear in s:s=s.replace(old_ai_clear,new_ai,1)
elif old_ai_plain in s:s=s.replace(old_ai_plain,new_ai,1)
else:raise SystemExit('Could not replace AI handler')

old_loop="function loop(){const x=energy();draw(audio.currentTime||0,x.energy,x.bass,x.mids,x.highs);raf=requestAnimationFrame(loop)}"
if old_loop not in s: old_loop="function loop(){const x=energy();draw(audio.currentTime||0,x.energy,x.bass);raf=requestAnimationFrame(loop)}"
new_loop="function loop(){if(!audio.paused&&audio.currentTime>=clipStart+clipLength){audio.pause();cancelAnimationFrame(raf);draw(clipLength);return}const x=energy();draw(Math.max(0,(audio.currentTime||clipStart)-clipStart,x.energy,x.bass,x.mids,x.highs);raf=requestAnimationFrame(loop)}"
if old_loop not in s: raise SystemExit('Could not replace render loop')
...
if old_preview not in s: raise SystemExit('Could not replace preview handler')
s=s.replace(old_preview,new_preview,1)

s=s.replace("audio.currentTime=0;cancelAnimationFrame(raf);loop();rec.start(500);await audio.play();const dur=Math.max(1,audio.duration||30),start=performance.now();","audio.currentTime=clipStart;cancelAnimationFrame(raf);loop();rec.start(500);await audio.play();const dur=clipLength,start=performance.now();",1)
...
print('Applied compatible 30–50 second selector with compact 16 kHz mono AI clip.')
